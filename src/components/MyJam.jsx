...
export function MyJam() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    position.set(latest * 200);
  });

  const negPosition = useMotionValue(0);

  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    negPosition.set(latest * -200);
  });
  ...
  return (
    <>
      ...
      >
        <motion.div style={{ position: "relative", left: position }}>
          Let's
        </motion.div>
        <motion.div style={{ position: "relative", left: negPosition }}>
          Jam
        </motion.div>
      </div>
      <div style={{ height: "200vh", width: "50vw", background: "#eee" }}></div>
